...
class SpanHierarchyBuilder {
	...
	build(records) {
		...
		for (let i = 0; i < recordsWithSpanId.length; i++) {
			...
			const node = this.#createSpanNode(record);
		...
		for (let i = 0; i < roots.length; i++) {
			const subtreeDepth = this.#calculateDepth(roots[i], 0);
		...
		for (let i = 0; i < roots.length; i++) {
			this.#sortChildrenByStartTime(roots[i]);
		}
	...
	#sortChildrenByStartTime(node) {
		...
		for (let i = 0; i < node.children.length; i++) {
			this.#sortChildrenByStartTime(node.children[i]);
		}
	}
}

export { SpanHierarchyBuilder };
