/**
 * @fileoverview
 * TimelineCalculator — чистая функция для расчёта временной шкалы (ось X с метками).
 *
 ...
 * @type {number[]}
 */
const NICE_INTERVALS_MS = Object.freeze([
	0.001,
	0.002,
	0.005, // < 1 ms
	0.01,
...
function calculateTickInterval(durationMs) {
	...
	return (
		NICE_INTERVALS_MS.find(function (interval) {
 ...
 */
function formatTimestamp(timestampMs, durationMs) {
 ...
 */
function calculateTimeline(items, config) {
	// Шаг 1: Определение границ шкалы
	// Берём минимальный и максимальный X из LayoutItem[],
	// конвертируем X → timestampMs через обратное преобразование.
	// Поскольку X = (timestampMs - timelineStart) * pixelsPerMs,
	// а timelineStart — это timestamp самого левого элемента,
	// мы сначала находим timestamp-границы по record.timestampMs.
	...
	for (var i = 1; i < items.length; i++) {
		var ts = items[i].record.timestampMs;
		if (ts < timelineStart) {
			timelineStart = ts;
		}
		if (ts > timelineEnd) {
	...
	var durationMs = timelineEnd - timelineStart;
	...
	var timelineWidth = (config && config.timelineWidth) || 1200;
	var pixelsPerMs = timelineWidth / durationMs;

	// Шаг 3: Автоматический подбор интервала меток
	var tickInterval = calculateTickInterval(durationMs);
